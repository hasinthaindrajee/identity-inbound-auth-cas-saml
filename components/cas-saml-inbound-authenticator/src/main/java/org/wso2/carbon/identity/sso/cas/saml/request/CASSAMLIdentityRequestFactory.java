...
import org.wso2.carbon.identity.sso.cas.saml.ProtocolConstants;
...
public class CASSAMLIdentityRequestFactory extends HttpIdentityRequestFactory {

    private static Log log = LogFactory.getLog(CASSAMLIdentityRequestFactory.class);

    @Override
    public String getName() {
        return "CASSAMLIdentityRequestFactory";
    }

    @Override
    public boolean canHandle(HttpServletRequest request, HttpServletResponse response) {
        String targetUrl = request.getParameter(ProtocolConstants.SAML_SERVICE_PROVIDER_ARGUMENT);
        return StringUtils.isNotBlank(targetUrl);
    }
    ...
    public IdentityRequest.IdentityRequestBuilder create(HttpServletRequest request,
        ...
        if (!StringUtils.isEmpty(targetUrl)) {
            builder = new CASServiceValidateSAMLRequest.CASServiceValidateSAMLRequestBuilder(request, response);
            ((CASServiceValidateSAMLRequest.CASServiceValidateSAMLRequestBuilder) builder).setLocale(request);
        } else {
